...
static gboolean update_tilda_window_size (gpointer user_data)
{
    ...
    gdouble newWidth = windowWidth;
    gdouble newHeight = windowHeight;
    ...
    if (ABS (configured_geometry.width - windowWidth) >= 1.0) {
        newWidth = configured_geometry.width;
    }

    if (ABS (configured_geometry.height - windowHeight) >= 1.0) {
        newHeight = configured_geometry.height;
    }

    gtk_window_resize (GTK_WINDOW (tw->window),
                       (gint) round(newWidth),
                       (gint) round(newHeight));
